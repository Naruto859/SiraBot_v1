...
export function validateConfigObjectWithPlugins(raw: unknown):
  ...
  // Warn about unvalidated custom model entries
  const customModels = config.models?.customModels;
  if (customModels && typeof customModels === "object") {
    for (const [modelKey, entry] of Object.entries(customModels)) {
      if (!entry.validated) {
        warnings.push({
          path: `models.customModels.${modelKey}`,
          message: `custom model "${entry.name}" has not been validated — run connection check before use`,
        });
      }
    }
  }
